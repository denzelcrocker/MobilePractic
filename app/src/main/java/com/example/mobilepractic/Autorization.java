...
public class Autorization extends AppCompatActivity {
    ...
    public void Profile(View v){
        if (email.getText().toString().equals("")||password.getText().toString().equals("")){
            Toast.makeText(Autorization.this,"Обязательные поля не заполнены", Toast.LENGTH_SHORT).show();
        }
        else{
            ...
            if (bool){
                ...
                Toast.makeText(Autorization.this, "Проверьте поле email", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    public void SingIN(){
        ...
        UserModelMask modelProf = new UserModelMask(emailstr, passwordstr);
        ...
        call.enqueue(new Callback<Mask>() {
            ...
            public void onResponse(Call<Mask> call, Response<Mask> response) {
                if (!response.isSuccessful()){
                    Toast.makeText(Autorization.this, "Пользователя с такой почтой нет", Toast.LENGTH_SHORT).show();
                    return;
                }
                if(response.body() != null) {
                    if (response.body().getToken()!=null){
                        saveInfo();
                        users = response.body();
                        Intent intent = new Intent(Autorization.this, MainPage.class);
                        Bundle b = new Bundle();
                        intent.putExtras(b);
            ...
            public void onFailure(Call<Mask> call, Throwable t) {
                Toast.makeText(Autorization.this,"Error: " + t.getMessage(), Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    public void Registr(View v) {
        Intent intent = new Intent(Autorization.this, Registration.class);
        startActivity(intent);
    }
}